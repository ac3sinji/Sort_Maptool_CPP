...
namespace ws {
    ...
    State Generator::createStartFromInitial(const InitialDistribution* initial) {
        ...
        // 템플릿 + startMixed OFF => 템플릿의 색 배치를 그대로 시작점으로 사용한다.
        // goal 배치(단색 병)에서 시작하면, 모든 병이 가득 찬 템플릿에서는 legal move만으로
        // 끝까지 단색 병 상태가 유지되어 "섞인" scramble 재생이 보이지 않을 수 있다.
        if (base && !opt.startMixed && !initial) {
            State st = *base;
            st.p = p;
            st.B.resize(p.numBottles);
            for (auto& b : st.B) {
                b.capacity = p.capacity;
                if ((int)b.slots.size() > p.capacity) {
